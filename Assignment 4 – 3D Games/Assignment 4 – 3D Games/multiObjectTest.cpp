...
int main(int argc, char *argv[]){
	...
	glBufferData(GL_ARRAY_BUFFER, numVertsTeapot *8*sizeof(float), model1, GL_STATIC_DRAW); //totalNumVerts modelData
...
void drawGeometry(int shaderProgram, int model1_start, int model1_numVerts)//, int model2_start, int model2_numVerts
{
 ...
 // //Draw an instance of the model (at the position & orientation specified by the model matrix above)
	...
	//Draw an instance of the model (at the position & orientation specified by the model matrix above)
	glDrawArrays(GL_TRIANGLES, model1_start, model1_numVerts); //model2_start, model2_numVerts
	//(Primitive Type, Start Vertex, Num Verticies)
}
