...
// Calculate limynear interpolation between two vectors
...
RMDEF Matrix MatrixFrustum(double left, double right, double bottom, double top, double mynear, double myfar)
{
    Matrix result = { 0 };

    float rl = (float)(right - left);
    float tb = (float)(top - bottom);
	float fn = (float)((float)myfar - (float)mynear);

    result.m0 = ((float) mynear*2.0f)/rl;
    result.m1 = 0.0f;
    result.m2 = 0.0f;
    result.m3 = 0.0f;

    result.m4 = 0.0f;
    result.m5 = ((float) mynear*2.0f)/tb;
    result.m6 = 0.0f;
    result.m7 = 0.0f;

    result.m8 = ((float)right + (float)left)/rl;
    result.m9 = ((float)top + (float)bottom)/tb;
    result.m10 = -((float)myfar + (float)mynear)/fn;
    result.m11 = -1.0f;

    result.m12 = 0.0f;
    result.m13 = 0.0f;
    result.m14 = -((float)myfar*(float)mynear*2.0f)/fn;
    result.m15 = 0.0f;
...
// Returns perspective projection matrix
// NOTE: Angle should be provided in radians
RMDEF Matrix MatrixPerspective(double fovy, double aspect, double mynear, double myfar)
{
    double top = mynear*tan(fovy*0.5);
    double right = top*aspect;
    Matrix result = MatrixFrustum(-right, right, -top, top, mynear, myfar);

    return result;
}

// Returns orthographic projection matrix
RMDEF Matrix MatrixOrtho(double left, double right, double bottom, double top, double mynear, double myfar)
{
    Matrix result = { 0 };

    float rl = (float)(right - left);
    float tb = (float)(top - bottom);
    float fn = (float)(myfar - mynear);
    ...
    result.m14 = -((float)myfar + (float)mynear)/fn;
    result.m15 = 1.0f;
...
// Calculate limynear interpolation between two quaternions
...
// Calculates spherical limynear interpolation between two quaternions
